(function () {
  var historicalChartVisualization = angular.module('redash.historical_visualization');

  historicalChartVisualization.config(['HistoricalVisualizationProvider', function (HistoricalVisualizationProvider) {
    var renderTemplate = '<historical-chart-renderer options="visualization.options" historical-query-result="historicalQueryResult"></historical-chart-renderer>';
    var editTemplate = '<historical-chart-editor options="visualization.options" historical-query-result="historicalQueryResult"></historical-chart-editor>';
    ...
    HistoricalVisualizationProvider.registerVisualization({
  ...
  historicalChartVisualization.directive('historicalChartRenderer', function () {
    return {
      restrict: 'E',
      scope: {
      ...
      controller: ['$scope', function ($scope) {
        ...
        var reloadData = function() {
          if (angular.isDefined($scope.historicalQueryResult)) {
            $scope.chartSeries = _.sortBy($scope.historicalQueryResult.getChartData($scope.options.columnMapping),
                                          function(series) {
        ...
        $scope.$watch('historicalQueryResult && historicalQueryResult.getData()', reloadData);
      }]
    };
  });

  historicalChartVisualization.directive('historicalChartEditor', function (ColorPalette) {
    return {
      restrict: 'E',
      ...
      link: function (scope, element, attrs) {
        ...
        var refreshColumns = function() {
          scope.columns = scope.historicalQueryResult.getColumns();
        ...
        var refreshColumnsAndForm = function() {
          refreshColumns();
          if (!scope.historicalQueryResult.getData() || scope.historicalQueryResult.getData().length == 0 || scope.columns.length == 0)
            return;
        ...
        var refreshSeries = function() {
          var seriesNames = _.pluck(scope.historicalQueryResult.getChartData(scope.options.columnMapping), 'name');
        ...
        scope.$watch('options.columnMapping', function() {
          if (scope.historicalQueryResult.status === "done") {
            refreshSeries();
          }
        }, true);

        scope.$watch(function() {return [scope.historicalQueryResult.getId(), scope.historicalQueryResult.status];}, function(changed) {
